...
isLegal (N (x,y)) (t,w,b) (i,j) = if notTaken (N (x,y)) w b (i,j) then checkKnight x y i j  else False

isLegal (B (x,y)) (t,w,b) (i,j) = if notTaken (B (x,y)) w b (i,j) then if  ((charToNum x) - y) == ((charToNum i)-j) then checkDiag  x y w b i j  else  if ((charToNum x) + y) == ((charToNum i)+j)  then checkAntiDiag x y w b i j else False else False

isLegal (Q (x,y)) (t,w,b) (i,j) = if notTaken (Q (x,y)) w b (i,j) then if j==y then checkCol x i w b j else if x == i then checkRow y j w b i else if  ((charToNum x) - y) == ((charToNum i)-j) then checkDiag  x y w b i j  else  if ((charToNum x) + y) ==  ((charToNum i)+j)  then checkAntiDiag x y w b i j else False else False

isLegal (K (x,y)) (t,w,b) (i,j) = if notTaken (K (x,y)) w b (i,j) then checkAdj x y i j else False

isLegal (P (x,y)) (t,w,b) (i,j) = if x==i then if notTakenBlock j i w b  then  checkPawn (P (x,y)) w b i j else False else if taken i j (P (x,y)) w b then checkPawnAttk (P (x,y)) w b i j else False
